...
mocks:
	...
	docker run --rm \
		...
		-w /work \
		-v ${PWD}:/work \
		vektra/mockery:v2.14.1 -r --keeptree --dir api/v1 --output api/v1/mocks --all

	docker run --rm \
		--user $$(id -u):$$(id -g) \
		-w /work \
		-v ${PWD}:/work \
		vektra/mockery:v2.14.1 -r --keeptree --dir pkg/datastore --output pkg/datastore/mocks --all
