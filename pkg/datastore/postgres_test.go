...
func TestCRUD(t *testing.T) {
	...
	filter["id"] = "tenant-1"
	err = ds.Find(ctx, filter, nil, &tenants)
	assert.NoError(t, err)
...
func TestFind(t *testing.T) {
	...
	filter := make(map[string]interface{})
	filter["id"] = t6
	err = ds.Find(ctx, filter, nil, &tfr)
	assert.NoError(t, err)
	assert.NotNil(t, tfr)
	assert.Len(t, tfr, 1)
